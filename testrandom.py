...
def main(argv=None):
    ...
    parser.add_argument('-n','--string',nargs='*', default='', help='read in a string')
    ...
    elif args.join:  # join all lines in a text file
        randin=""
        for bits in args.infile.readlines():
            randin+=bits.strip()
        out+=performrndtest(randin)
 
    elif len(args.string) > 0: # read in string instead
        out+=performrndtest(args.string)
